...
from .fpn_eca import FPN
import torch.nn as nn
import torch
from .config import DefaultConfig

from .backbone.convnext3fenzhi import ConvNeXt
...
class FCOS(nn.Module):
    ...
    def __init__(self, config=None):
        ...
        self.modelator = Modulator()


    def forward(self, img_z, box_z, img_x, box_x=None, z_f=None, flag=False):
        '''
        Returns
        ...
        '''
        if z_f != None and flag:
            z = z_f
        elif z_f != None:
            z = self.extract_feat(z_f)
            return z
        else:
            z = self.extract_feat(img_z)
        x = self.extract_feat(img_x)
        modu_feat, loss = self.modelator(z, box_z, x, box_x)
        # return modu_feat[-1]
        cls_logits, cnt_logits, reg_preds = self.head(modu_feat)
        return [cls_logits, cnt_logits, reg_preds, loss]

    def extract_feat(self, img):
        C2, C3, C4, C5 = self.backbone(img)
        all_P = self.fpn([C2, C3, C4, C5])  # a list len:6 contain:(16*256*72*72),(16*256*36*36)...
        return  all_P
...
class DetectHead(nn.Module):
    ...
    def coords_fmap2orig(self, feature, stride):
        '''
    ...
    def _reshape_cat_out(self, inputs, strides):
        ...
        for pred, stride in zip(inputs, strides):
            ...
            coord = self.coords_fmap2orig(pred, stride).to(device=pred.device)
...
class FCOSDetector(nn.Module):
    ...
    def forward(self, inputs):
        ...
        batch_imgz, batch_boxz, batch_imgx,z = inputs
        out = self.fcos_body(batch_imgz, batch_boxz, batch_imgx,None,z,True)
        scores, classes, boxes = self.detection_head(out[:-1])
        boxes = self.clip_boxes(batch_imgx, boxes)
        return scores, classes, boxes
